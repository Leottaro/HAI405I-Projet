...
class SampleBot(Bot):
    ...
    def getCardToPlay(self, game):
        ...
        cartePossibles = [
            carte
            for carte in deck
            if carte not in self.hand and carte not in game.alreadyPlayedCards
        ]
        ...
        for echantillon in range(self.nEchantillons):
            ...
            if echantillon > 104 and 2 * min(scoreCartes) < sum(scoreCartes) / len(
                scoreCartes
            ):
                break
            ...
            for maCarteI in range(len(self.hand)):
                for carteOther in coupsAdverse:
                    ...
                    cards = sorted(
                        [self.hand[maCarteI], carteOther], key=lambda x: x.value
                    )
                    for card in cards:
