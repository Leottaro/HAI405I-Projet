...
function randomString(length, minCharCode, maxCharCode) {
  ...
  return [...Array(length)]
    .map((_) =>
      String.fromCharCode(Math.floor(minCharCode + Math.random() * range)),
    )
    .join("");
}
...
io.on("connection", function (socket) {
  ...
  socket.on("reqSignIn", async (json) => {
    const [err, rows] = await sqlRequest(
      `SELECT * FROM account WHERE pseudo="${json.pseudo}"`,
    );
    if (err) throw err;
  ...
  socket.on("reqLogIn", async (json) => {
    if (
      Object.values(sockets).some(
        (socketInfo) => socketInfo.compte === json.pseudo,
      )
    ) {
      return socket.emit("resSignIn", {
        success: false,
  ...
  socket.on("reqCreate", async (json) => {
    ...
    if (
      nbrJoueursMax < jeux.playersRange[0] ||
      nbrJoueursMax > jeux.playersRange[1] ||
      !jeux
    ) {
    ...
    do {
      ...
      [err, rows] = await sqlRequest(
        `SELECT * FROM partie where code="${code}"`,
      );
  ...
  socket.on("reqJoin", (code) => {
    ...
    if (!jeux.addPlayer(socket.id)) {
      return socket.emit("resJoin", {
        success: false,
        message: jeux.started
          ? "cette partie à commencée"
        ...
        jeux.setRoundCallback(() => {
          for (const playerID of jeux.playersIDs) {
            if (!jeux.choosed[playerID]) {
              const i = Math.floor(
                Math.random() * jeux.paquets[playerID].length,
              );
  ...
  socket.on("reqProfilStat", async () => {
    ...
    const [err, rows] =
      await sqlRequest(`SELECT nomJeux, place, points FROM aJoue, partieFinie 
        WHERE codeR=code
