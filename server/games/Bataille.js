...
class Bataille {
  ...
  start() {
    if (
      this.ended ||
      this.started ||
      this.playersIDs.length < Bataille.playersRange[0]
    ) {
      return false;
    }
  ...
  nextRound() {
    ...
    for (let i = 1; i < sortedChoosed.length; i++) {
      if (
        this.choosed[sortedChoosed[i]].valeur ==
        this.choosed[sortedChoosed[0]].valeur
      ) {
    ...
    this.choosed = {};
    this.tempCartes = [];
    this.enLice = this.playersIDs.filter(
      (playerID) => this.paquets[playerID].length > 0,
    );
