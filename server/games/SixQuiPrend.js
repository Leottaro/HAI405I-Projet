...
class SixQuiPrend {
  ...
  start() {
    if (
      this.ended ||
      this.started ||
      this.playersIDs.length < SixQuiPrend.playersRange[0]
    ) {
      return false;
    }
  ...
  coup(playerID, carte) {
    if (
      ...
      !this.paquets[playerID].some((carteJson) =>
        Carte.equals(carteJson, carte),
      )
    ) {
      return false;
    }
  ...
  everyonePlayed() {
    return this.playersIDs.every(
      (playerID) =>
        this.paquets[playerID].length == 0 || this.choosed[playerID],
    );
  }
  ...
  nextRound() {
    ...
    for (const playerID of players) {
      ...
      for (const ligne in this.plateau) {
        ...
        if (
          carte.valeur < choosed.valeur &&
          carte.valeur > biggest.carte.valeur
        ) {
          biggest = { ligne, carte };
        }
      }
    ...
    if (
      this.playersIDs
        .map((id) => this.paquets[id])
        .some((paquet) => paquet.length == 0)
    ) {
      this.started = false;
      this.plateau = [[], [], [], []];
      this.start();
    ...
    if (this.playersIDs.some((id) => this.scores[id] >= 66)) {
      ...
      Object.keys(this.paquets).forEach(
        (playerID) => (this.paquets[playerID] = []),
      );
      this.choosed = {};
      this.plateau = [[], [], [], []];
      this.endCallback();
