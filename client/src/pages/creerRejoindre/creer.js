...
function Creer() {
    ...
    socket.on("resGamesInfos", (json) => { // quand jeux === sixQuiPrend { roundDelays: {min, default, max}, choiceDelays: {min, default, max} }
        ...
        <div id="CRContent">
            ...
            {jeux === "sixQuiPrend"
                ? <div id="ParametersDiv">
                    <SixParam desc="temps max par rounds" min={roundDelays.min} defaultValue={roundDelays.default} max={roundDelays.max} onChange={(value) => options.roundDelay = value} />
                    <SixParam desc="temps max choix de ligne" min={choiceDelays.min} defaultValue={choiceDelays.default} max={choiceDelays.max} onChange={(value) => options.choiceDelay = value} />
                </div>
                : <></>}
